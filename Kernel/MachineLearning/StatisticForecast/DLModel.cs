...
namespace MachineLearning {
    ...
    internal class DLModel {
        ...
        internal DLModel(StatForecast Model) {
            this.model = Model;
        ...
        internal void SetParamPosition(string Name, int Property, int v) {
            DLMParameter p = this.GetParam(Name, Property);
        ...
        internal void SetParamDistributions(double[,] Mean, double[,] Var) {
            ...
            foreach (ArrayList ListOfParams in this.PolSea)
                if (ListOfParams != null)
                    foreach (DLMParameter p in ListOfParams)
                        if (p.IsActive()) {
        ...
        internal int GetNActiveParams() {
            ...
            foreach (ArrayList ListOfParams in this.PolSea)
                if (ListOfParams != null)
                    if (((DLMParameter)ListOfParams[0]).IsActive()) NParams += ListOfParams.Count;

            return (NParams);
        }

        internal int GetNParamsPol() {
            if (((DLMParameter)this.PolSea[0][0]).IsActive()) return (((ArrayList)this.PolSea[0]).Count);
            else return (0);
        }
        ...
        internal int GetNActiveParamsPol() {
            ...
            if (this.PolSea[0] != null) {
                foreach (DLMParameter p in this.PolSea[0])
                    if (p.IsActive()) np++;
                return np;
            }
            else return 0;
        ...
        internal int GetNActiveParamsSea() {
            ...
            for (int i = 1; i < this.PolSea.Length; i++)
                if (((DLMParameter)this.PolSea[i][0]).IsActive())
                    NParams += ((ArrayList)this.PolSea[i]).Count;
        ...
        internal bool RestrictModelToNObs() {
            ...
            if (!((DLMParameter)this.PolSea[0][0]).IsActive()) Haschanged = true;
            this.ActivatePol();
            ValidObsTmp -= 2;
            ...
            for (int i = this.PolSea.Length - 1; i > 0; i--)
        	{
                if (ValidObsTmp <= 0)
                    break;
                DLMParameter p = (DLMParameter)this.PolSea[i][0];
        ...
        internal int[] GetActiveRows() {
            ...
            foreach (ArrayList ListOfParams in this.PolSea)
                if (((DLMParameter)ListOfParams[0]).IsActive()) ActRows[((DLMParameter)ListOfParams[0]).GetParamNumber()] = 1;
        ...
        internal void CalcGMatrix() {
            if (this.status_G == CompStatus._Pending) {
                ...
                foreach (ArrayList ListOfParams in this.PolSea) {
                    if (((DLMParameter)ListOfParams[0]).IsActive()) {

                        index = ((DLMParameter)ListOfParams[0]).GetParamNumber();
                        switch (((DLMParameter)ListOfParams[0]).GetParamType()) {
                            case (CompType._Pol):
                            ...
                            case (CompType._Sea):
                                ...
                                int armnumber = ((DLMParameter)ListOfParams[0]).GetArmonicNumber();
        ...
        internal int ApplyDiscFactors(double[,] M, double[] DF, bool CalcFcst) {
            ...
            foreach (ArrayList ListOfParams in this.PolSea) {
                p = ((DLMParameter)ListOfParams[0]);
                if (p.IsActive()) {
                    if (p.GetParamType() == CompType._Pol) row = 0;
                    else row = 1;
                    initial = p.GetParamNumber();
                    foreach (DLMParameter x in ListOfParams) {
                        if (CalcFcst)
        ...
        internal void ActivateArmonic(int NARM) {
            if (NARM == -1)
                ...
                foreach (DLMParameter p in this.PolSea[NARM])
                    p.SetActive();
            this.ReCalcParamNumber();
        ...
        internal void AddCompPol() {
            ...
            if (this.grade == 1)
                this.PolSea[0].Add(new DLMParameter("Level", CompType._Pol, 1));

            if (this.grade == 2) {
                this.PolSea[0].Add(new DLMParameter("Level", CompType._Pol, 2));
                this.PolSea[0].Add(new DLMParameter("Trend", CompType._Pol, 2));
            }
        }
        ...
        internal bool AddCompSea(int NARM) {
            ...
            if (NARM == this.maxarm)
                this.PolSea[NARM].Add(new DLMParameter("Cos", CompType._Sea, NARM));
            else {
                this.PolSea[NARM].Add(new DLMParameter("Cos", CompType._Sea, NARM));
                this.PolSea[NARM].Add(new DLMParameter("Sin", CompType._Sea, NARM));
            }
            return true;
        }
