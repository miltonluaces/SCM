...
namespace MachineLearning {
    ...
    internal class Norm {
        ...
        internal Norm(TS X) : this(X, 1, 0, 1) {
        }
      
        internal Norm(TS X, double factor, double minFactor, double maxFactor) {
            this.factor=factor;
        ...
        internal TS Normalize(TS X) {
            TS nX = (X - this.xMin) / this.xRange;
            TS fnX = nX * this.factor;
            return fnX;
        }

        internal TS UnNormalize(TS nX) {
            TS X = (nX / this.factor) * this.xRange + this.xMin;
            return X;
        }
